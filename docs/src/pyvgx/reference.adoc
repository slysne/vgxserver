...
==== Graph Structure
Graphs are built from two basic object types: <<vertexobject, _vertices_>> (nodes) and _arcs_ (directed edges). Vertices are <<connect_func, connected _explicitly_ via arcs>> or _implicitly_ via their similarity (represented by a vector metric) to other vertices.

* _Explicit connections_ are realized internally as _adjacency vectors_ directly associating vertex pairs. Index-free adjacency allows efficient movement from node to node by following direct links. This network of vertices connected by arcs is the graph.
* _Implicit connections_ are expressed via <<vectorobject, _similarity vectors_>>. Vertices with such vectors assigned to them become indirectly associated by a chosen vector metric such as <<_euclideandistance, Euclidean distance>> or <<_cosine, Cosine similarity>>. A proximity graph can be constructed to facilitate efficient vector search, using the explicit network to form navigable paths between implicitly connected items.

Two vertices may be <<connect_func, connected>> by a _simple arc_ (one connection) or a _multiple arc_ (many connections). A vertex may be connected to any number of other vertices.

Two explicitly connected vertices are _neighbors_. The set of arcs between a vertex and all its neighbors is the vertex _neighborhood_.

The total number of explicit connections a vertex has is the vertex _degree_. Outbound arcs and inbound arcs are counted as _outdegree_ and _indegree_, respectively.

The graph's _size_ is the sum of all degrees for all vertices in the graph, i.e. the total number of explicit connections. The total number of vertices in a graph is the graph _order_.


Both vertices and arcs may have _properties_. <<_setproperty_2, Vertices properties>> are freely assignable key-value pairs. xref:specification/arcSpecificationSyntax.adoc#arcelements[Arc properties] include a _relationship_ type, _modifier_ (value type), and a _numeric value_. Queries use these properties for filtering, scoring, sorting and display of search results. 
 
==== Graph Search
Graph search can be _global_ or _local_. Global queries scan the entire graph to return any matching vertices or arcs. Local queries scan the neighborhood around a selected anchor vertex.

<<graph/graphQuery.adoc#graphneighborhood, Neighborhood queries>> can be nested to extend several levels beyond the immediate neighbors of the anchor. Independent filters and collection parameters can be specified for each level. Results can be sorted by any attribute or property, or according to a user-definable rank score. Traversal filters, ranking formulas, and result field selectors all use a common expression language which allows mathematical formulas of any complexity to be executed.

<<graph/graphNavigationQuery.adoc#, Neighborhood navigation queries>> can be used to automatically explore the entire graph from any starting vertex. This is especially useful for traversing proximity graphs where nodes have been connected based on their mutual vector similarity. Traversal of such graphs form the basis of highly efficient vector search, and can be combined with any other filtering to implement hybrid semantic search. 

==== Implicit Connections
A vertex becomes implicitly connected to other vertices by setting the vertex _vector_. <<vectorobject, Vectors>> are numeric arrays or lists of weighted features.

The implicit connection strength between two vertices with vectors is determined by their vector similarity. When configured in <<system_initialize_func, Euclidean Vector>> mode similarity can be computed either as the _Euclidean distance_ or the _Cosine similarity_ between vectors. When configured in <<system_initialize_func, Feature Vector>> mode similarity can be computed as the _Cosine similarity_ or the _Jaccard index_, or a combination of the two.
...
==== Capacity
All graph data is held in memory, ensuring low latency and predictable performance (soft realtime) for all operations. The system may hold any number of independent graphs of any size and order at the same time, limited only by the available memory on your machine. Graphs may be <<save_func, saved>> to disk and restored later.

==== Object Expiration
Vertices and arcs may be assigned <<_setexpiration, expiration timestamps>> (time-to-live or TTL) for automatic deletion at a future point in time. Expiration times are independently assignable for all objects. TTL has a resolution of one second for any expiration timestamp ranging from imminent to infinity.
...
A <<service/pluginadapter.adoc#, plugin framework>> allows remote execution of Python functions that you define and <<service/pluginadapter.adoc#system_addplugin_func, register>> with the system. Plugins are invoked by sending HTTP requests to <<service/service.adoc#, VGX Server>>, a fully asynchronous, multithreaded request engine running within the VGX core. This server is independent from the Python interpreter. 
...
* <<graphobject_methods>>
** <<createvertex_func, CreateVertex()>>
** <<connect_func, Connect()>>
** <<graph/graphQuery.adoc#graphneighborhood, Neighborhood()>>
...
[[system_exitrunserver_func]]`pyvgx.*system.ExitRunServer*()`::
Causes <<system_runserver_func, RunServer()>> to exit.
+
...
Block forever until interrupted by `signal.SIGINT` or by another thread calling <<system_exitrunserver_func, ExitRunServer()>>.
+
...
[[countdefinitions_func]]`*CountDefinitions*()`::
Return the number of pre-defined expressions (See <<evaluator/evaluator.adoc#evaluatordefine, Define()>>.)
...
[[isdefined_func]]`*IsDefined*( _name_ )`::
Return True if the named expression has been defined with <<evaluator/evaluator.adoc#evaluatordefine, Define( "_name_ := ..." )>>, otherwise return False.
...
[[isgraphlocal_func]]`*IsGraphLocal*()`::
...
[[memory_counters_attr]]`<<evaluator/evaluator.adoc#evaluatormemory, *counters*>>`::
Navigation progress counters, returns tuple: (_score_evaluations_, _threshold_contributions_, _accepted_into_frontier_, _accepted_into_result_, _recursion_depth_, _node_expansions_, _node_visits_, _node_revisits_avoided_).

[[memory_order_attr]]`<<evaluator/evaluator.adoc#evaluatormemory, *order*>>`::
The memory order (readonly) is the log2 of its size. The memory object size is always a power of two and will be returned by `len( memobj )`.
...
[[memory_order_vector]]`<<evaluator/evaluator.adoc#evaluatormemory, *vector*>>`::
Memory probe vector (read and write). Must be a `pyvgx.Vector`. Accessible as `M.vector` in the expression language.
...
[[memory_object_vsetadd]]`<<evaluator/evaluator.adoc#evaluatormemory, *VSetAdd*>>()`::
Add a vertex (address) to <<evaluator/evaluator.adoc#evaluator_mappingsets, vset>> in the memory object.

___

==== VSetClear

[[memory_object_vsetclear]]`<<evaluator/evaluator.adoc#evaluatormemory, *VSetClear*>>()`::
Clear the contents of any <<evaluator/evaluator.adoc#evaluator_mappingsets, vset or iset>> in the memory object.
