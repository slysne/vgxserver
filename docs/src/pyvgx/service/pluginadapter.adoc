...
[TIP]
====
Install `orjson` for better performance with JSON-heavy payloads
...
IMPORTANT: Any engine or dispatcher (other than the <<response_attr_toplevel, topmost dispatcher>>) must return a <<pyvgx_pluginresponse, PluginResponse>> object. Partition merge requires PluginResponse objects to function, since merging arbitrary object types is generally undefined.

TIP: A *pre*-processor may terminate a request early (e.g. a cache-hit) by returning a PluginResponse object.

TIP: A *post*-processor may re-submit a request by returning a (special) PluginRequest object obtained from <<pluginresponse_resubmittable, PluginResponse.Resubmittable()>>.
...
// cspell:ignore maxhits sortby keytype
...
+
Explicitly define the _keytype_ as _int_, _float_, _bytes_ or _str_. Must be comparable with _sortby_ if provided.
+
...
|anchor:response_attr_meta[]`*meta*`
|_object_ or None
|r/w
a|Any object to supplement the response entries list. When matrix back-end responses are merged in a dispatcher, only partition 0 meta object is kept. Back-end meta objects are returned all the way up to the topmost dispatcher unless deliberately overwritten (or removed) along the way.
...
// cspell:ignore requestmethod
|anchor:response_attr_requestmethod[]`*requestmethod*`
|_str_
|readonly
|"GET" or "POST"

// cspell:ignore requestpath
...
Transmission of PluginResponse objects between back-end engines and dispatchers is using an internal, binary protocol. However, a topmost dispatcher or engine returning a PluginResponse object automatically renders the response as JSON. A top dispatcher plugin may choose to return an object of a different type, such as a simple dict or list, or any other Python object.

TIP: Plugin code can check <<response_attr_toplevel, response.toplevel>> to determine if it is the topmost dispatcher for a request, and then return an object of any type if needed.

IMPORTANT: Responses returned from any engine or dispatcher other than the topmost dispatcher *must* be PluginResponse objects, since dispatchers cannot merge or processes arbitrary Python objects.

.PluginResponse as JSON
[source, JSON]
----
{
  "status": "OK",
