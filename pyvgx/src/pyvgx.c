...
static char * __get_banner_bar( unsigned indent ) {
  ...
  CString_t *CSTR__python = __banner_entry( indent, "PYTHON", "%s", Py_GetVersion() );
  if( CSTR__build && CSTR__python ) {
    int sz = minimum_value( maximum_value( CStringLength(CSTR__build), CStringLength(CSTR__python) ), 120 );
    for( int i=0; i<sz; i++ ) {
      bar[i] = '_';
    }
  }
  iString.Discard( &CSTR__build );
  iString.Discard( &CSTR__python );
  return bar;
}
