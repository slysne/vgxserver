...
Request Engine:: HTTP server metrics

Transaction Interconnect:: VGX Transaction Service metrics

Graph Engine:: Graph objects and property storage summary

Memory:: System memory usage information
...
|`*Down*`
|Instance does not respond to status requests and is either not running or not reachable. Intermittent occurrences of this status may appear under heavy load.
|`*Readonly*`
...
|`host:port`
|Hostname (or IP) and *admin* port of instance. Subtract 1 from admin port to get main service/plugin port.
...
|`*Delta*`
|Dispatcher's back-end matrix has one or more partitions with inconsistent replicas, or back end subscriber instance is out of sync with its provider
|`*Degraded*`
|Dispatcher's back-end matrix has one or more unavailable replicas (reduced query capacity)
|`*Memory*`
|Memory warning: Physical host memory usage is 81% - 88%
|`*Mem _x_%*`
|High memory warning: Physical host memory usage is 88% - 94%
|`*! MEM _x_%*`
|Critically high memory: Physical host memory usage is above 94%
|`*NoEngine*`
|All engine partition(s) unavailable in dispatcher back-end matrix (or <<multinode.adoc#descriptor_topology_dispatch_partial_results, partial matrix>> not allowed)
...
|`**__m__**R`
|Proxy Dispatcher's back-end replica count _m_
|`**__m__**R x **__n__**P`
|Matrix Dispatcher's back-end dimensions (_m_ replicas, _n_ partitions)
|<empty>
|N/A
...
Call these endpoints to perform administrative tasks. Obtain a new, unique <<admin_service_authtoken, _authtoken_>> before making a request.
...
|Generate a new, unique authtoken to be submitted with the next admin request
...
|Initiate instance shutdown. This requires *two* authtokens. Obtain the first token and pass in `authshutdown` parameter. Then obtain the second token and pass in `authtoken` parameter.
 +
 +
 Optionally pass `persist=1` to save graphs on shutdown.
 +
 +
 Optionally pass `restartable=1` to cause shutdown to internally invoke <<../reference.adoc#system_exitrunserver_func, pyvgx.system.ExitRunServer()>> instead of process-wide exit signal SIGTERM.
 +
 +
 Optionally pass a copy of the first admin token (the one passed to `authshutdown`) in `authfullreset` parameter to fully erase all graph data before shutdown. (`persist=1` is implied in this case.)
...
TIP: Use the special identifier `@` to mean all instances. (Often more convenient than `*` in case you forget to add quotes, causing trouble with shell expansion.)

The same option can occur multiple times. Actions are performed sequentially and synchronously, i.e. one action has to complete before the next action starts.
...
==== `--bind <id>` (`-B`)
Start the transaction input service on the <<multinode.adoc#descriptor_instance_tport, tport>> defined for this instance. If no tport is defined this command has no effect.
...
==== `--pausein <id>` (`-p`)
Pause transaction input service on subscriber instance(s). Transaction data will still be received and queued by subscribers, but not processed while in the paused state. Use <<vgxadmin_option_resumein, --resumein>> to re-enable processing of received transaction data.
...
==== `--pauseout <id>` (`-P`)
Pause transaction output emitter on provider instance(s). Transaction data is produced and queued, but not sent to subscribers while in the paused state. Use <<vgxadmin_option_resumeout, --resumeout>> to start sending data to subscribers again.
...
==== `--pausettl <id>` (`-t`)
Pause TTL event processor. Objects with expiration time set will not be deleted while in the paused state. Use <<vgxadmin_option_resumettl, --resumettl>> to start processing TTL events again.
...
If adding new plugins the `<pd>` file format is the JSON equivalent of the Python <<multinode.adoc#vgxinstance_plugin_definitions, plugin definition list>> used with <<multinode.adoc#pyvgx_vgxinstance_startinstance_func, pyvgx.VGXInstance.StartInstance()>>.
...
[[vgxadmin_option_reset]]
==== `--reset <id>` (`-A`)
Reset and restart service, if application supports this action. Resetting a service means erasing all graph data and persisting the empty system, before attempting restart.

For restart to work the application must be implemented as explained for <<vgxadmin_option_restart, vgxadmin --restart>>.

.Fully reset instance "B1" to an empty state
[.copyable]
[source, shell]
----
vgxadmin --reset "B1"
...
[[vgxadmin_option_restart]]
==== `--restart <id>` (`-e`)
Restart service, if application supports this action. 

Supporting restart requires the application to re-enter <<../reference.adoc#system_runserver_func, pyvgx.system.RunServer()>> in a loop after exit caused by the restart request.
Before calling RunServer() again the application must also start the HTTP server, since it automatically shuts down by this request.

If not implemented this way the restart request will terminate the service.
...
===== Example: Minimal restartable application
...
==== `--serviceout <id>` (`-i`)
Set instance(s) HTTP server to the S-OUT state. Plugin requests on the <<service.adoc#server_a_and_server_b, main port>> will return HTTP code <<service.adoc#status_503_service_out, "503 Service out">>. 
