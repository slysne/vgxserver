[[graphquerymethods]]
= Graph Query Methods <<graph.adoc#, icon:arrow-circle-up[title="pyvgx.Graph"]>> <<graphRecursionQuery.adoc#, icon:arrow-circle-right[title="Recursion Query"]>> <<theend, icon:chevron-circle-down[title="Bottom of page"]>>
:toc: left
:toclevels: 3
:sectnums:
:sectnumlevels: 5
...
[source, python]
----
pyvgx.Graph.Neighborhood( id[, arc[, pre[, filter[, post[, neighbor[, vector[, collect[, recursion[, result[, fields[, select[, rank[, sortby[, aggregate[, memory[, offset[, hits[, timeout[, limexec ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] )
----
...
|_recursion_
|<bool> or <dict> +
 See <<neighborhood_recursion>>
|False
|Enable recursive search mode and specify recursion behavior
...
A neighborhood search starts at the specified vertex _id_ (anchor) and radiates outwards via its arcs to its immediate neighbors. If requested, the search may continue from all neighbors along _their_ arcs to reach farther into the graph. Separate conditions for vertices and arcs are specified at each level (distance) from the anchor. A simple neighborhood query only traverses the anchor's arcs to its immediate neighbors.
...
.Collect the names of all neighbors of anchor vertex
[source, python]
----
g.Neighborhood(
...
.Restrict search by arc's relationship type
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
.Further restrict search by terminal condition
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
.Control what fields are returned and their format
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
.Return names of anchor's terminals which are themselves connected to vertices meeting a certain condition
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
.Further restrict the returned items to those who like someone who knows someone who visited some country
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
.Perform item collection at another level of nesting
[source, python]
----
g.Neighborhood(
    id = anchor_name,
...
It is possible to perform value aggregation of collected arcs, rather than returning individual arcs. Arcs matching the arc filter criteria and sharing the same head vertex (i.e. have the same terminal) are collapsed into an _aggregated arc_ with a value representing some aggregation of the individual arc values. Arcs will be aggregated even if they are collected from different neighborhoods in a nested traversal scenario, so long as their heads terminate at the same vertex.
...
Recursive search is a powerful feature for automatically exploring a graph to collect top matching items ranked by some scoring formula. A common use case is approximate nearest neighbor (ANN) search. Given a sufficiently connected proximity graph the built-in recursive search algorithm can traverse its way towards the best matches using only a probe vector as search condition.
...
See separate <<graphRecursionQuery.adoc#, Neighborhood Recursion Query>> section for more details.
...
<<graphRecursionQuery.adoc#, icon:arrow-circle-right[size=2x, title="Recursion Query"]>>
