...
Request Engine HTTP service <<service.adoc#server_a_port, main port>>

[[descriptor_instance_prefix]]HTTP Service Prefix: `instances.Si.*prefix*`:: [Default=`None`] +
Request Engine HTTP service <<service.adoc#system_starthttp_func__prefix, path prefix>> for URI aliases.
[CAUTION]
====
...
[IMPORTANT]
====
An instance without subscribers will be marked as <<../reference.adoc#isgraphlocal_func, local only>>. This flag is persistent and is _not_ automatically cleared if the topology is later changed to include subscribers for an instance. See <<local_only_caution, how to reset local only>> for more details.
====
...
[CAUTION]
====
Sometimes graph(s) in a VGX instance may be marked as <<../reference.adoc#isgraphlocal_func, local only>>, preventing graph data from being replicated to attached subscribers. This is almost always caused by re-using saved instances that were originally created with a different <<system_descriptor, descriptor>>. For example, a `builder` instance may originally have had no subscribers defined in its <<descriptor_topology_transaction, transaction topology>>, marking it local only. Even if one or more `search` instances are later added to the descriptor, the `builder` still remembers its original local-only role.

The easiest way to reset the local only state is to remove the following file from the affected instance's data directory. (Stop instance before removing file, then restart.)

----
vgxroot/
├─ graphname/
    ├─ vxgraph_[graphname].dat   ( <- remove to reset state )
----
