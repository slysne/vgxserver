...
[[pluginresponse_class]]`_class_ *pyvgx.PluginResponse*( **[**_maxhits_**[**, _sortby_**]]** )`::
...
+
Calls to `Append()` must provide a _sortkey_ compatible with the response object's _sortby_ configuration, otherwise `pyvgx.ResponseError` is raised. A compatible _sortkey_ is one with the <<../constants/sortSpecificationConstants.adoc#, expected type>> and a value consistent with <<../constants/sortSpecificationConstants.adoc#, sort direction>>.
+
