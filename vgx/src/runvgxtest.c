...
static int run_selected_test( const char *testroot, const char *prefix ) {
  ...
  XTRY {
    if( CharsStartsWithConst( prefix, vgx_prefix ) ) {
      ...
      if( (selected = get_selected_tests( query, names )) == NULL ) {
        THROW_ERROR( CXLIB_ERR_GENERAL, 0x002 );
      }
      if( vgx_unit_tests( selected, testroot ) != 0 ) {
      ...
      if( comlib_unit_tests( selected, testroot ) != 0 ) {
      ...
      if( iFramehash.test.RunUnitTests( selected, testroot ) != 0 ) {
